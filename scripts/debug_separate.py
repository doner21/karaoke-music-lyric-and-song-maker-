...
# dora might not be installed or importable easily if it's internal to demucs repo structure not exposed. 
# actually demucs depends on dora-search? No, likely just printed 'fatal'.
# I'll replace fatal with print+exit.
...
# Replace relative imports with absolute imports assuming demucs is installed
...
def log_config(tag, key, value):
    print(f"[CONFIG_DIFF] {tag} | {key}: {value}")
...
def main(opts=None):
    ...
    log_config("ARGS", "device", args.device)
    log_config("ARGS", "shifts", args.shifts)
    log_config("ARGS", "overlap", args.overlap)
    log_config("ARGS", "split", args.split)
    log_config("ARGS", "segment", args.segment)
    log_config("ARGS", "stem", args.stem)
    log_config("ARGS", "int24", args.int24)
    log_config("ARGS", "float32", args.float32)
    log_config("ARGS", "clip_mode", args.clip_mode)
    
    log_config("ENV", "torch_version", th.__version__)
    log_config("ENV", "cuda_available", th.cuda.is_available())
    log_config("ENV", "default_dtype", th.get_default_dtype())
    
    if th.cuda.is_available():
        log_config("ENV", "cudnn_version", th.backends.cudnn.version())
        log_config("ENV", "cudnn_benchmark", th.backends.cudnn.benchmark)
        log_config("ENV", "cudnn_deterministic", th.backends.cudnn.deterministic)
    
    # Model info
    if isinstance(model, BagOfModels):
        log_config("MODEL", "type", "BagOfModels")
        log_config("MODEL", "models_count", len(model.models))
    else:
        log_config("MODEL", "type", type(model).__name__)
        if hasattr(model, 'segment'):
             log_config("MODEL", "default_segment", model.segment)
    ...
    for track in args.tracks:
        ...
        # LOGGING APPLY ARGS
        print(f"[DEBUG] Calling apply_model with device={args.device}, shifts={args.shifts}, overlap={args.overlap}")
        ...
        log_config("EXPORT", "kwargs", kwargs)
